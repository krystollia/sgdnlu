...
class BertUnifiedLabeler(tf.keras.Model):
  ...
  instantiates both a span labeling network based on a single dense layer and
  a classification network based on the passed `num_classes`.

  Attributes:
    network: A transformer network. This network should output a sequence output
      and a classification output. Furthermore, it should expose its embedding
      table via a "get_embedding_table" method.
    num_classes: the number of classes that classification model on [CLS].
  ...
  def __init__(self,
    ...
    super(BertUnifiedLabeler, self).__init__(
